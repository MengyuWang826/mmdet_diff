...
if __name__ == '__main__':
    ...
    coarse_json = 'data/refine_annotations/maskrcnn_val.json'
    ...
    with tqdm(total=len(all_imgs)) as pbar:
        for img_id in all_imgs:
            if img_id in coarse_dts:
                ...
                for ins_id, ann in enumerate(ann_info):
                    ...
                    if cat_id in refine_dts[img_id] and cat_id in coarse_dts[img_id]:
                        ...
                        ious_c = [cal_iou(gt_mask, _) for _ in coarse_masks]
                        idx_c = ious_c.index(max(ious_c))
                        ious_f = [cal_iou(gt_mask, _) for _ in refine_masks]
                        idx_f = ious_f.index(max(ious_f))
                        iou_c = round(max(ious_c), 3)
                        iou_f = round(max(ious_f), 3)
                        filename=f'results_100/{img_id}_{ins_id}_c{iou_c}_f{iou_f}.png'
                        output_save(image, gt_mask, coarse_masks[idx_c], refine_masks[idx_f], filename=filename)
            pbar.update()
    a = 1
