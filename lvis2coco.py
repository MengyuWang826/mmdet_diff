...
def cocofy_lvis(input_filename, coco_val_file, output_filename):
    """
    ...
    with open(coco_val_file, "r") as f1:
        coco_val = json.load(f1)
    
    lvis_annos = lvis_json["annotations"]
    cocofied_lvis = {}
    ...
    coco_val_images = set()
    for image in coco_val["images"]:
        coco_val_images.add(image['id'])
    
    new_image_list = []
    for image in lvis_json["images"]:
        if image['id'] in coco_val_images:
            new_image_list.append(image)
    cocofied_lvis["images"] = new_image_list

    new_annos = []
    ann_id = 1
    for ann in lvis_annos:
        if ann['image_id'] not in coco_val_images:
            continue
    ...
    cocofied_lvis['info'] = lvis_json['info']
    cocofied_lvis['licenses'] = lvis_json['licenses']

    print(len(lvis_annos))
    print(len(new_annos))
...
if __name__ == "__main__":
    cocofy_lvis(
        "data/lvis_annotations/lvis_v1_val.json",
        "data/coco_annotations/instances_val2017.json",
        "lvis_v1_val_cocofied.json"
    )