...
log_level = 'INFO'
load_from = None
resume_from = 'checkpoints/iter_2000.pth'
workflow = [('train', 1000)]
...
model = dict(
    ...
    diffusion_betas=dict(type='linear', start=0.999, stop=0, num_timesteps=6),
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys=dict(
            prompt_encoder=dict(lr_mult=10), mask_decoder=dict(lr_mult=0.1))))
