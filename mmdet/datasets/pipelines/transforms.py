...
class Resize:
    ...
    def __init__(self,
        ...
        if mask_scale_factor is None:
            self.mask_scale_factor = 1
        else:
    ...
    def _resize_masks(self, results):
        ...
        if self.mask_scale_factor == 1:
            ...
            for key in results.get('mask_fields', []):
                if results[key] is None:
                    continue
                results[key] = results[key].rescale(results['mask_scale'])
    ...
    def __call__(self, results):
        ...
        if 'scale' not in results:
            ...
            if not self.override:
                ...
                self._random_scale(results)
        if self.mask_scale_factor != 1:
            mask_scale = (int(results['scale'][0] * self.mask_scale_factor), int(results['scale'][1] * self.mask_scale_factor))
            results['mask_scale'] = mask_scale
...
class DoubleResize:
    ...
    def __init__(self,
                 ...
                 interpolation='bilinear',
                 override=False):
        assert large_scale is not None
        self.large_scale = large_scale
        assert small_scale is not None
    ...
    def _resize_img(self, results):
        """Resize images with ``results['scale']``."""
        key = 'img'
        large_img, scale_factor = mmcv.imrescale(
            results[key],
            self.large_scale,
            return_scale=True,
            interpolation=self.interpolation,
        ...
        # the w_scale and h_scale has minor difference
        # a real fix should be done in the mmcv.imrescale in the future
        new_h, new_w = large_img.shape[:2]
        h, w = results[key].shape[:2]
        w_scale = new_w / w
        h_scale = new_h / h

        small_img, _, _ = mmcv.imresize(
            results[key],
            self.small_scale,
            return_scale=True,
            interpolation=self.interpolation,
        ...
        results['img'] = large_img
        results['small_img'] = small_img

        scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                dtype=np.float32)
        results['img_shape'] = large_img.shape
        # in case that there is no padding
        results['pad_shape'] = large_img.shape
        results['scale_factor'] = scale_factor
        results['keep_ratio'] = True
        results['img_fields'].append('small_img')
    ...
    def _resize_masks(self, results):
        ...
        for key in results.get('mask_fields', []):
            if results[key] is None:
                continue
            results[key] = results[key].resize(self.small_scale)
...
class Pad:
    ...
    def _pad_masks(self, results):
        ...
        if 'mask_scale' in results:
            pad_shape = results['mask_scale']
        else:
...
class MatchGTandDT:
    def __init__(self,
        ...
        self.expand_aug = expand_aug
        self.max_out = max_out
        self.gt_norm = gt_norm
    
    def _cal_iou(self, mask1, mask2):
    ...
    def _pre_coarse_and_gt(self, results):
        new_coarse_masks = []
        new_gt_masks = []
        h, w = results['img_info']['height'], results['img_info']['width']
        gt_masks, areas, gt_labels = results['gt_masks'].masks, results['gt_masks'].areas, results['gt_labels']
        if len(results['coarse_masks']):
            coarse_masks = results['coarse_masks'].masks
            dt_labels = np.array(results['dt_labels'])
            for idx, label in enumerate(gt_labels):
                gt_mask = gt_masks[idx]
                area = areas[idx]
                label_match = dt_labels == label
                if label_match.any():
                    coarse_masks_ = coarse_masks[label_match]
                    ious = [self._cal_iou(gt_mask, _) for _ in coarse_masks_]
                    ious = np.array(ious)
                    matched_idx = ious >= self.iou_thr
                    if matched_idx.any():
                        matched_coarse_masks = coarse_masks_[matched_idx]
                        matched_num = len(matched_coarse_masks)
                        chosen_idx = np.random.choice(matched_num, size=1)
                        new_coarse_masks.extend(matched_coarse_masks[chosen_idx])
            ...
            for idx, mask in enumerate(gt_masks):
                new_coarse_masks.append(generate_block_target(mask, areas[idx]))
                new_gt_masks.append(mask)
        results['gt_masks'] = BitmapMasks(new_gt_masks, h, w)
        results['coarse_masks'] = BitmapMasks(new_coarse_masks, h, w)
        del results['gt_labels']
        del results['dt_labels']
        return results


    def __call__(self, results):
        results = self._pre_coarse_and_gt(results)
        return results
