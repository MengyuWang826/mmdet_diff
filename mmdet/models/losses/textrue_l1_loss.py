...
class TextureL1Loss(nn.Module):
    ...
    def __init__(self, kernel_size=3, eps=1e-4, texture_time_steps=[0, 1], reduction='mean', loss_weight=1.0):
        super(TextureL1Loss, self).__init__()
        self.reduction = reduction
        self.loss_weight = loss_weight
        self.kernel_size = kernel_size
        self.sobel = SobelOperator(eps)
        self.texture_time_steps = texture_time_steps
    ...
    def forward(self,
        ...
        # print(pred_texture.min())
        # print(pred_texture.max())
        # print(target_texture.min())
        # print(target_texture.max())
        # texture_save(pred_texture, target_texture)

        weight = torch.zeros_like(pred_texture)
        for t in self.texture_time_steps:
            weight[time_steps==t] = 1
