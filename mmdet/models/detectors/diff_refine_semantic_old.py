...
class OldDiffRefineSemantic(BaseRefinementor):
    def __init__(self,
                 task,
                 denoise_model,
                 diffusion_cfg,
    ...
    def simple_test_semantic(self, img_metas, img, coarse_masks, **kwargs):
        ...
        # global_step
        foreground_img, foreground_mask, foreground_coor = self._get_global_input(img, coarse_masks, ori_shape, current_device)
        foreground_mask, _ = self.p_sample_loop([(foreground_mask, foreground_img, None)], 
                                                        global_indices, 
                                                        current_device, 
                                                        use_first_step_flag=True,
                                                        use_last_step_flag=True)
        foreground_mask = foreground_mask >= 0.5
        img_mask = _do_paste_mask(foreground_mask, foreground_coor, img_metas)

        # object_step
        object_imgs, object_masks, object_coors = self._get_object_input(img, img_mask, ori_shape, current_device)
        ...
        # local_step
        patch_imgs, patch_masks, patch_coors = self._get_local_input(img, img_mask, local_indices, ori_shape)
        ...
        local_masks, _ = self.p_sample_loop(xs, 
                                            ...
                                            use_first_step_flag=True,
                                            use_last_step_flag=True)
        mask = self.paste_local_patch(local_masks, img_mask, patch_coors)
    ...
    def _get_global_input(self, img, coarse_masks, ori_shape, current_device):
        ...
        fg_pad_width = self.test_cfg.get('fg_pad_width', 0)
        do_blur = self.test_cfg.get('do_blur', False)
        model_size = self.test_cfg.get('model_size', 256)
        coarse_mask = coarse_masks[0].masks[0]
        x_1, y_1, x_2, y_2 = mask2bbox(coarse_mask)
        x_1_fg, x_2_fg = self._get_object_coor(x_1, x_2, img_w, fg_pad_width)
        y_1_fg, y_2_fg = self._get_object_coor(y_1, y_2, img_h, fg_pad_width)
        foreground_img = img[:, :, y_1_fg: y_2_fg, x_1_fg: x_2_fg]
        foreground_img = F.interpolate(foreground_img, size=model_size, mode='nearest')
        foreground_mask = torch.tensor(coarse_mask[y_1_fg: y_2_fg, x_1_fg: x_2_fg], device=current_device,dtype=torch.float32)
        foreground_mask = F.interpolate(foreground_mask.unsqueeze(0).unsqueeze(0), size=model_size)
        foreground_mask = (foreground_mask >= 0.5).float()
        if do_blur:
            foreground_img = self.uniform_blur(foreground_img, current_device)
        return foreground_img, foreground_mask, (x_1_fg, y_1_fg, x_2_fg, y_2_fg)
        
    def _get_object_input(self, img, img_mask, ori_shape, current_device):
        img_h, img_w = ori_shape
        ...
        mask = img_mask[0].cpu().numpy().astype(np.uint8)
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        for cnt in contours:
            x_1, y_1, w, h = cv2.boundingRect(cnt)
            if w > size_thr and h > size_thr:
                x_1_ob, x_2_ob = self._get_object_coor(x_1, x_1+w, img_w, ob_pad_width)
                y_1_ob, y_2_ob = self._get_object_coor(y_1, y_1+h, img_h, ob_pad_width)
    ...
    def _get_object_coor(self, x_1, x_2, w, pad_width):
        x_1_ob = max(x_1 - pad_width, 0)
        x_2_ob = min(x_2 + pad_width, w)
    ...
    def _get_local_input(self, img, img_mask, local_indices, ori_shape):
