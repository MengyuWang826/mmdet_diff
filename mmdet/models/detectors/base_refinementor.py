...
class BaseRefinementor(BaseModule):
    """Base class for detectors."""
    def __init__(self,
                 task=None,
                 init_cfg=None):
        super(BaseRefinementor, self).__init__(init_cfg)
        assert task is not None, 'undefine refinement task'
        self.task = task
    ...
    def forward(self, img_metas, return_loss=True, **kwargs):
        ...
        if return_loss:
            ...
            if self.task == 'instance':
                ...
                return self.simple_test_matting(img_metas, **kwargs)
            else:
    ...
    def simple_test_matting(img_metas, **kwargs):
        raise NotImplementedError
